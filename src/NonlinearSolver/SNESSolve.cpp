...
PetscErrorCode computeJacobian(SNES snes,Vec U,Mat Jac,Mat B,void *ctx){
    ...
    if(Jac!=B){ //Hf: I think this judgement is redundant, Jac!=B is true definitely.
...
bool SNESSolver::solve(Mesh &mesh,DofHandler &dofhandler,FE &fe,
    ...
    // Hf: m_rhs.getVectorCopy() and m_amatrix.getCopy() just return data's handler, not a deep copy                           
