...
void BCSystem::applyDirichletBC(const FECalcType &calctype,
    ...
    for(const auto &name:bcnamelist){
        ...
        for(e=eStart;e<eEnd;e++){
            ...
            for(i=1;i<=m_local_elmtinfo.m_nodesnum;i++){// Hf: bc element's m_nodesnum
                ...
                switch (bctype)
                {
                case BCType::DIRICHLETBC:// Hf: globaldofids: vector of global dof data id of constraint dof (BCBlock and node specified)
